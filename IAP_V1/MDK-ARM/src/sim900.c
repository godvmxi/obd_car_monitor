...
{	
	...
	char *p;
		...
		Send_AT_And_Wait("ATE1\r","OK",500);
...
//}IAP_BRIFE;	//IAP 参数返回
	iapBrife.sizeOfApp = 100;
	iapBrife.appCrc = 0xffff;
	iapBrife.hv = 1;
	iapBrife.sv = 1;
	iapBrife.start[0] = '#';
	iapBrife.start[1] = '#';
	iapBrife.packetNum = 0;
	iapBrife.crc = calBufCrc(iapBrife.start+4,sizeof(IAP_BRIFE)-4);


//int16_t updateFlashConfig(SOCKET *net,SOCKET *iap,IAP_BRIFE *iapBrife,BLUE_CONFIG *blue,OBD_CONFIG *obd)
	updateFlashConfig(&net,&iap,&iapBrife,&blue,NULL);
	#ifdef PRINTF_DEBUG
